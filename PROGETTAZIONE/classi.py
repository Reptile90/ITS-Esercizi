...
class Nazione:
    ...
    def remove_citta(self, citta: Citta) -> None:
        if citta in self._citta:
            self._citta.remove(citta)
            if citta.nazione() == self:
                citta._nazione = None
        # TODO da implementare
        pass
...
class Citta:
    ...
    def set_nazione(self, nazione: Nazione) -> None:
        if hasattr(self, "_nazione") and self._nazione is not None:
            if self._nazione != nazione:
                self._nazione.remove_citta(self)
        self._nazione = nazione
        if self not in nazione._citta:
